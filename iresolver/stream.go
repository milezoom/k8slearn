	...
	for {
		...
		if err != nil {
			...
			case io.ErrUnexpectedEOF:
				grpclog.Infof("iresolver: Unexpected EOF during watch stream event decoding: %v", err)
			default:
				grpclog.Infof("iresolver: Unable to decode an event from the watch stream: %v", err)
			}
			return
		}
		sw.result <- obj
